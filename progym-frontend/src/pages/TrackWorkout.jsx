...
const workoutOptions = {
  Chest: ['Push-Ups', 'Incline Push-Ups', 'Decline Push-Ups', 'Clap Push-Ups', 'Archer Push-Ups', 'Chest Dips',
  ...
  Biceps: ['Bicep Curl', 'Hammer Curl', 'Concentration Curl', 'Incline Dumbbell Curl', 'Zottman Curl', 'Cross-Body Hammer Curl',
  ...
  Triceps: ['Triceps Dips', 'Close-Grip Push-Ups', 'Diamond Push-Ups', 'Pike Push-Up to Triceps Extension', 'Wall Triceps Extensions',
  ...
  Back: ['Pull-Ups', 'Inverted Rows', 'Superman Hold', 'Reverse Snow Angels', 'Wall Slides', 'Table Rows',
  ...
  Legs: ['Bodyweight Squats', 'Jump Squats', 'Lunges', 'Bulgarian Split Squats', 'Step-Ups', 'Wall Sit', 'Glute Bridges', 'Calf Raises',
  ...
  Shoulders: ['Pike Push-Ups', 'Handstand Push-Ups', 'Wall Walks', 'Arm Circles', 'Plank to Downward Dog',
  ...
  Abs: ['Crunches', 'Sit-Ups', 'Leg Raises', 'V-Ups', 'Flutter Kicks', 'Mountain Climbers', 'Russian Twists', 'Planks', 'Hollow Body Hold', 'Dead Bug',
...
const TrackWorkout = () => {
  ...
  const handleSave = async () => {
    ...
    const newWorkout = { category, exercise: selectedExercise, ...inputData };
  ...
  const radarData = bodyParts.map(group => ({
    group,
    volume: workouts.filter(w => w.category === group)
                    .reduce((sum, w) => sum + w.sets * w.reps * w.weight, 0),
  }));

  const pieData = radarData.map(d => ({ name: d.group, value: d.volume }));
  const scatterData = workouts.map(w => ({ weight: Number(w.weight), reps: Number(w.reps) }));
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.fixedCategoryHeader}>
        ...
        <div style={styles.gridBodyParts}>
          {bodyParts.map((part) => (
            ...
            >
              <img
                src={`/images/bodyparts/${part.toLowerCase()}.png`}
                alt={part}
                style={styles.bodyImage}
              />
              <div style={styles.bodyLabel}>{part}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {selectedExercise && (
        <div style={styles.inputPanel}>
          ...
          <div style={styles.inputGroupPremium}>
            {['sets', 'reps', 'weight', 'date'].map((field) => (
              <input
                key={field}
                name={field}
          ...
          <button
            ...
            style={{ ...styles.button, ...(hoverSave ? styles.buttonHover : {}) }}
          >
            Save Workout
          </button>
      ...
      <WorkoutCharts
        data={filteredData.map(d => ({ ...d, volume: d.sets * d.reps * d.weight }))}
        radarData={radarData}
        pieData={pieData}
